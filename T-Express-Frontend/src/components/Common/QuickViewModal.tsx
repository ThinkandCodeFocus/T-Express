  ...
  const thumbnails = useMemo(() => {
    ...
    if (product) {
      ...
      if (product.image_principale) {
        images.push(resolveBackendImageUrl(product.image_principale, fallbackImage));
      }
      ...
      if (product.images) {
        ...
        parsedImages.forEach((img: string) => {
          // `resolveBackendImageUrl` laisse passer une URL deja absolue :
          // l'API en renvoie, et les prefixer une seconde fois produisait des
          // liens du type /storage/https://.../storage/... donc des 404.
          const url = resolveBackendImageUrl(img, fallbackImage);
          if (img && !images.includes(url)) {
            images.push(url);
          }
        });
      }
            ...
            <div className="max-w-[445px] w-full">
              ...
              <div className="flex flex-wrap items-center gap-5 mb-6">
                <div className="flex items-center gap-1.5">
                  {/* Etoiles calculees sur la note reelle, et masquees tant qu'aucun
                      avis n'existe : le gabarit en affichait cinq en dur, dont
                      quatre pleines, meme sur un produit jamais note. */}
                  {reviewsCount > 0 && (
                    <div className="flex items-center gap-1">
                      {[1, 2, 3, 4, 5].map((cran) => (
                        <svg
                          key={cran}
                          className={cran <= Math.round(rating) ? "fill-[#FFA645]" : "fill-gray-4"}
                          width="18"
                          height="18"
                          viewBox="0 0 18 18"
                          fill="none"
              ...
              <div className="flex flex-wrap items-center gap-4">
                <BoutonCommanderWhatsApp
                  produitId={product?.id ?? productRedux?.id ?? ''}
                  nom={productName}
                  prix={prixPromo}
                  prixInitial={prixPromo < prix ? prix : null}
                  image={previews[activePreview]}
                  quantite={quantity}
                ...
                <button
                  ...
                  className={`inline-flex font-medium text-dark bg-gray-1 border border-gray-3 py-3 px-7 rounded-md ease-out duration-200 hover:border-blue hover:text-blue
                  `}
                >
                  Ajouter au panier
                </button>
