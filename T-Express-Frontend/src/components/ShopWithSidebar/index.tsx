...
const ShopWithSidebar = () => {
  ...
  const [filters, setFilters] = useState({
    ...
    tri: "recent" as 'prix_asc' | 'prix_desc' | 'nom_asc' | 'nom_desc' | 'recent' | 'populaire'
  });
  ...
  const loadCategories = async () => {
    try {
      const categories = await categorieService.getListe();
      const cats = categories.map((cat: Categorie) => ({
        name: cat.nom,
        products: cat.produits_count || 0,
        isRefined: false,
        id: cat.id
      }));
      setCategories(cats);
  ...
  const handleSortChange = (value: string) => {
    // Convertir le format "tri:direction" en format API
    const triMap: Record<string, typeof filters.tri> = {
      'prix:asc': 'prix_asc',
      'prix:desc': 'prix_desc',
      'nom:asc': 'nom_asc',
      'nom:desc': 'nom_desc',
  ...
  const handlePriceFilter = (min: number | null, max: number | null) => {
    setFilters(prev => ({ ...prev, prix_min: min, prix_max: max }));
  };
              ...
              <div className="rounded-lg bg-white shadow-1 pl-3 pr-2.5 py-2.5 mb-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    ...
                    <p>
                      Affichage <span className="text-dark">{products.length} sur {pagination.total}</span>{" "}
                      Produits
                    </p>
                  </div>
